...
export const mutations = {
  addItem: (state: State, id: number) => {
    const item = state.products.find((product) => product.id === id);
    return state.myRentals.push(item as Product);
  },
  removeItem: (state: State, id: number) => {
    const i = state.myRentals.map((item) => item.id).indexOf(id);
    return state.myRentals.splice(i, 1);
  },
};
